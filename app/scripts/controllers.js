...
/**
 *
 * smartdust company angularjs function list
 */



//////////////////////////////////////////////
function  shownodeCtrl($scope) {
    $scope.Nodes= {
        Nodedis:[
            { Id: '6', Class: '分补',Size:'20kvar',capaA: 1, capaB: 0,capaC:1},
        ],
        Nodecom: [
            { Id: '1', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
            { Id: '2', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
            { Id: '3', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
            { Id: '4', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
            { Id: '5', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
            { Id: '6', Class: '共补',Size1:'20kvar',Size2:'20kvar',capa1: 1, capa2: 0},
        ]

     }
}
function settingCtrl($scope,ngmqtt){
    var options = {
        clientId: "test",
        protocolId: 'MQTT',
        protocolVersion: 4
    ...
    ngmqtt.listenConnection("settingCtrl", function(){

        console.log("connected");
        ngmqtt.subscribe('topic2test');

        var i = 0;
        $interval(function() {
            ngmqtt.publish('topic2test', i.toString());
            i++;
        }, 300);
    });

    // $scope.number=[overvoltageProtection,overcurrentProtection,PFCuplimit,PFCdownlimit,hiProtection,hvProtection];
    $scope.overvoltageProtection=1;
    $scope.overcurrentProtection=1;
    $scope.PFCuplimit=1;
    $scope.PFCdownlimit=1;
    $scope.hiProtection=1;
    $scope.hvProtection=1;
$scope.setting=function(){
 //   ngmqtt.publish('topic4test', [ $scope.overvoltageProtection, $scope.overcurrentProtection,$scope.PFCuplimit,$scope.PFCdownlimit,$scope//.hiProtection,$scope.hvProtection]);
      ngmqtt.listenConnection("settingCtrl", function(){

        console.log("connected");
        ngmqtt.publish('topic2test', $scope.overvoltageProtection.toString());
...
angular
    ...
    .controller('testController1', ['$scope', '$interval', 'ngmqtt', function($scope, $interval, ngmqtt){
       ...
       ngmqtt.connect('ws://localhost:9001', options);
     ...
     //       ngmqtt.subscribe('topic2test');
        ...
        ngmqtt.listenMessage("testController1", function(topic, message){
           ...
           $scope.lastName=topic;
            $scope.firstName=message;
        });
    }])
    .controller('testController2', ['$scope', '$interval', 'ngmqtt', function($scope, $interval, ngmqtt){
        var options = {
            clientId: "test",
            protocolId: 'MQTT',
            protocolVersion: 4
    ...
    .controller('testController3', ['$scope', '$interval', 'ngmqtt', function($scope, $interval, ngmqtt){
        var options = {
            clientId: "test",
            protocolId: 'MQTT',
            protocolVersion: 4
    ...
    .controller('settingCtrl', ['$scope', '$interval', 'ngmqtt', function($scope, $interval, ngmqtt){
    var options = {
        clientId: "test",
        protocolId: 'MQTT',
        protocolVersion: 4
   ...
   // $scope.hvProtection=1;
    $scope.setting=function(){
        ...
        ngmqtt.listenMessage("settingCtrl", function(topic, message){
            console.log("message received (settingCtrl)");
            console.log(topic + ": " + message);
            $scope.overvoltageProtection=message;
        });


    }])
.controller('MqttCtrl', function($scope, kmqtt) {
    var client = kmqtt.connect('ws://test.mosquitto.org:8080');

    client.on("message", function(topic, payload) {
        console.log(topic, payload.toString());
        $scope.lastName=topic;
        $scope.firstName=payload.toString();
    });
